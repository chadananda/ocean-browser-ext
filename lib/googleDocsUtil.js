var googleDocsUtil = function() {
    ...
    function getGoogleDocument() {
        ...
        // paragraphrenderers.onkeypress=function(){alert("hello!");};
    ...
    function setTextCurrentCursor(textt) {
        var caret, caretRect;
        var caretIndex = 0;
        var caretLineIndex = 0;
        var caretLine = 0;
                ...
                for (var k = 0; k < wordhtmlgeneratorWordNodes.length; k++) {
                    ...
                    if (caretRect) {
                        if (doesRectsOverlap(wordhtmlgeneratorWordNodeRect, caretRect)) {
                            ...
                            caretLine = lineCount;
                            var spanText = ""; 
                            spanText = $(wordhtmlgeneratorWordNodes[k]).text();
                            var llength = spanText.length;
                            var setText = spanText.slice(0,caretLineIndex)+textt+spanText.slice(caretLineIndex,llength);
                            $(wordhtmlgeneratorWordNodes[k]).text(setText);
                        }
                    }
    ...
    return {
        ...
        getUserCaretDom : function(){
            return getUserCaretDom();
        },
        setTextCurrentCursor : function(text){
            return setTextCurrentCursor(text);
        }
    }
}();
