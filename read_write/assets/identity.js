...
class GoogleAuthenticator extends _GoogleServices2.default {
    ...
    */
    ...
    /**
     * Sign out out of your Google Account
     */
    signOut() {
...
class IdentityManager {
    ...
    constructor(accountType) {
        ...
        this._defaultAccountType = accountType;
        ...
        chrome.identity.getProfileUserInfo(userInfo => {
            ...
            var userID = "";
            // if there is an email address sign in with google
        ...
        //this._authenticator = (new AuthenticatorFactory(accountType)).authenticator;
    ...
    */
     ...
     * @param {object} authenticator authenticator to use. eg Microsoft, Google. Defaults to current manager authenticator
     *                            if one isn't defined.       
     */
    getLicenseNoCache(interactive, callback, productCode) {
        var authenticator = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : this._authenticator;
        ...
        authenticator._getID(interactive, id => {
            ...
            authenticator._getLicense(id, license => {
    ...
    */
    getLicense(interactive, callback, productCode) {
        ...
        //let tempGoogleAuthenticator = (new AuthenticatorFactory(texthelp.RW4GC.enums.AccountType.GOOGLE)).authenticator; 
        //tempGoogleAuthenticator.getLocalID((id)=>{
        chrome.identity.getProfileUserInfo(userInfo => {

            if (userInfo.email.length > 0 && this._authenticator == null) {
                this._authenticator = new _AuthenticatorFactory2.default(texthelp.RW4GC.enums.AccountType.GOOGLE).authenticator;
            }

            if (this._authenticator == null && interactive == true) {
                ...
                chrome.tabs.query({
                    ...
                    if (thistab.url.includes("selectiondialog.html?q")) {

                        callback({
                            "error": "No id to license"
                        });
                        return;
                    } else {
                        ...
                        chrome.tabs.query({
                            ...
                            if (tabsArray[0] == null) {
                                ...
                                chrome.tabs.create({
                                    "url": "chrome-extension://" + chrome.runtime.id + "/assets/accountselection/selectiondialog.html?q=" + thistab.id
                                }, callback);
                                ...
                                chrome.tabs.update(tabsArray[0].id, updateProperties, tab => {});
            ...
            chrome.storage.managed.get('features', function (policy) {
                ...
                this._getDailyChromeSyncLicense((license, dateStamp) => {
                    ...
                    // otherwise we need to hit the licensing service. First get the users id
    ...
    getId(interactive, callback) {
        var authenticator = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : this._authenticator;
        ...
        authenticator._getID(interactive, callback);
    }
    ...
    collectHighlights(documentHighlights, callback) {
        ...
        authenticator._collectHighlights(documentHighlights, callback);
    }
    ...
    vocab(params, callback) {
        ...
        authenticator._vocab(params, callback);
    }
    ...
    getVoiceNoteOAuthToken(params, callback) {
        ...
        authenticator._getVoiceNoteOAuthToken(params, callback);
    }
    ...
    setProvider(provider) {
        var sendResponse = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
        ...
        setTimeout(function () {
            ...
            // create the authenticator with the right authentication services.

            var tempAuthenticator = null;
            if (provider == "MS") {

                tempAuthenticator = new _AuthenticatorFactory2.default(texthelp.RW4GC.enums.AccountType.MICROSOFTSHAREPOINT).authenticator;
            } else {
                tempAuthenticator = new _AuthenticatorFactory2.default(texthelp.RW4GC.enums.AccountType.GOOGLE).authenticator;
            }

            // get the license without caching
            this.getLicenseNoCache(true, license => {

                // if a valid license is not retrieved then signout.
                // This could be due to the sign in being canceled. 
                if (license == undefined) {
                    this.signOut(false);
                    return;
                }

                if (license.error !== undefined) {
                    this.signOut(false);
                    return;
                }

                this._authenticator = tempAuthenticator;

                // no response but still licensed so make sure 
                // not to sign out.
                if (sendResponse == null) {
                    return;
                }

                // otherwise refresh the UI with the new license.
                sendResponse({
                    'method': 'onRefreshLicense',
    ...
    /**
     * Signout from the current authenticator account. All caching info will be cleared.
     * @param {boolean} forceLogout if true hits the logout api of any authentication service.
     */
    signOut() {
        var forceLogout = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;


        if (forceLogout) {
            // Logout of the authenticator
            this._authenticator.signOut();
        }

        // Clear the authenticator and any cached licensing. 
        this._authenticator = null;
        this._license = null;
        chrome.storage.sync.set({
            thpl2: {}
        });
        // this._authenticator = (new AuthenticatorFactory(this._defaultAccountType)).authenticator;
    }
...
class MicrosoftSharepointAuthenticator extends _MicrosoftSharepointServices2.default {
    ...
    */
    ...
    /**
     * Sign out out of your Microsoft Sharepoint Account
     */
    signOut() {
    ...
    getAuthenticatedID(token, interactive, scopes, funcToOAuth, onFuncResponse, params, retry) {
        ...
        try {
            fetch(profileURL, {
                ...
                this._id = data.mail;
                if (this._id == null) {
                    this._id = data.userPrincipalName;
                }
